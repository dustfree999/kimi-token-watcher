...
- **多风格界面**：内置 5 套界面风格——暗色（默认）/ 浅色 / 暖白极简 / 战术终端 / 暗夜玻璃，侧栏下拉或设置页一键切换，选择本地记忆；图表 / 徽标配色随风格自动适配
- **定价目录全量重建**：内置 models.dev 全量价格目录，新增缓存写入价（cache_write）与订阅套餐折算标记
- **价格匹配算法升级**：分段匹配防子串吸附（如 `gpt-5.6-luna` 不再被 `gpt-5` 吸附）、众数价排除渠道加价异常、旧版同步目录自动回落
- 新增定价回归验证脚本 `tests/verify_pricing.mjs`
